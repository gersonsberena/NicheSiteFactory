...
export default function Footer({ config }: { config: CoachConfig }) {
  ...
  return (
    <footer style={{ background: "var(--bg)", borderTop: "1px solid rgba(var(--accent-rgb),0.2)" }}>
      <div className="max-w-[1200px] mx-auto px-6 md:px-10 py-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div>
          <a href="#top" className="font-bold text-[20px]" style={{ fontFamily: "Playfair Display, serif", color: "var(--text)" }}>{about.name}</a>
          <p className="text-[12px] mt-1" style={{ color: "var(--text-muted)", fontFamily: "Source Sans 3, sans-serif" }}>
        ...
        <p className="text-[11px]" style={{ color: "var(--text-muted)", fontFamily: "Source Sans 3, sans-serif" }}>© {year} {about.name}</p>
      </div>
    </footer>
  )
}
